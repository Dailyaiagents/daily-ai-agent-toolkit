...
1. `release-workflow`: the protected ref and exact commit digest from which
   `.github/workflows/release-evidence.yml` was loaded. It is first because the
   hosted GitHub attestation API was observed validating the first entry as the
   canonical Actions source dependency.
